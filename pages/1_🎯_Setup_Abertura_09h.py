...
========================================
Versão 3.6 - Com dois blocos de IA: SMC/ICT (imagens) + Pré-Abertura (texto)
"""
...
class ConfigSetup09:
    ...
    modelo_groq_padrao: str = "qwen/qwen3.6-27b"
    temperatura_groq: float = 0.3
    max_tokens_groq: int = 2500
    max_imagem_size: int = 350
...
CAMINHO_PROMPT_0900 = os.path.join(PROMPT_DIR, "Prompt_Abertura_0900.txt")
...
def executar_mapear_tendencias() -> bool:
    """Executa o MapearTendencia15Min.py para gerar análise de tendência."""
    try:
...
def garantir_tendencias() -> tuple[bool, str]:
    """Garante que o arquivo de tendências exista."""
...
def imagem_para_base64_otimizada(caminho_imagem, max_size=350):
    """Converte imagem para base64 com tamanho equilibrado."""
    try:
...
def carregar_imagens_otimizadas():
    """Carrega imagens com tamanho reduzido."""
    imagens = {}
    for nome, caminho in IMAGENS.items():
        if os.path.exists(caminho):
            b64, mime = imagem_para_base64_otimizada(caminho, CONFIG.max_imagem_size)
            if b64:
...
def carregar_todos_dados() -> Dict[str, Dict[str, Any]]:
    """Carrega todos os arquivos de entrada do pipeline."""
...
class SetupService:
    """Contém toda a lógica de decisão do Setup 09:00."""
    ...
    def dados_para_ia_resumido(self) -> Dict[str, Any]:
        """Retorna dados resumidos para IA (economiza tokens)."""
        s = self.sinal()
...
def montar_prompt_ia_smc(dados: Dict[str, Any]) -> str:
    """Prompt para análise SMC/ICT com DUAS imagens."""
    
    return f"""⚠️ RESPONDA EM PORTUGUÊS. NÃO MOSTRE RACIOCÍNIO.
...
🖼️ VOCÊ TEM DUAS IMAGENS:
1. WIN 1min: Price action recente (entrada)
2. WIN 5min: Estrutura e tendência (contexto)

RESPONDA EM PORTUGUÊS:

1. ESTRUTURA: (alta/baixa/lateral) - analise a 5min
2. ORDER BLOCKS: Onde estão? - analise as duas
3. FVG: Há lacunas? - analise as duas
4. LIQUIDEZ: Onde está? - analise as duas
5. CONFLUÊNCIA: Dados confirmam o gráfico?
6. ENTRADA IDEAL: Condição (use a 1min)
7. STOP/ALVO: Valores sugeridos
8. CONFIANÇA: 1-10
...
3. ANALISE AS DUAS IMAGENS:
   - WIN 1min: Price action, entrada, ordem de candles
   - WIN 5min: Estrutura, tendência, níveis principais
...
def render_bloco_ia_smc(service: SetupService):
    ...
    with st.expander("⚙️ Configurações SMC/ICT", expanded=not bool(groq_key)):
        ...
        modelo = st.selectbox(
            "Modelo (com visão)",
            ["qwen/qwen3.6-27b", "llama-3.2-11b-vision-preview"],
            index=0,
            key="modelo_smc"
        )
        st.caption("💡 qwen/qwen3.6-27b é o recomendado para análise de gráficos")
...
def render_bloco_ia_pre_abertura(service: SetupService):
    ...
    with st.expander("⚙️ Configurações Pré-Abertura", expanded=not bool(groq_key)):
        ...
        st.caption("💡 Modelos de texto são ideais para análise rápida de dados")
...
def main():
    ...
    st.markdown("---")
    st.caption("Setup Abertura 09:00 • v3.6 • Duas análises IA: SMC/ICT + Pré-Abertura")
