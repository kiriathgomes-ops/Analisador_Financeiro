...
def render_bloco_1_filtro_classificacao(service: SetupService):
    ...
    def velocimetro(valor: float, titulo: str) -> "go.Figure":
        """
        Velocímetro com 0 no CENTRO.
        O preenchimento (arco) parte do 0 para a esquerda (venda) ou direita (compra).
        Plotly preenche do mínimo por padrão — usamos dois traços base + indicador no valor.
        """
        v = max(-8.0, min(8.0, float(valor)))

        # Base: semicírculo de fundo via pie (buraco = gauge)
        # Setores de cor alinhados às faixas (em graus: 180° = esquerda/-8, 0° = direita/+8)
        # Plotly gauge nativo: range [-8,8] com 0 no meio do eixo.
        # Truque: bar transparente + threshold grosso no valor + steps coloridos.
        fig = go.Figure(
            go.Indicator(
                ...
                gauge={
                    "axis": {
                        ...
                        "ticktext": ["-8", "-4.5", "-2.5", "-1.5", "0", "1.5", "2.5", "4.5", "8"],
                    },
                    # Barra "apagada": não preencher desde -8
                    "bar": {"color": "rgba(0,0,0,0)", "thickness": 0.01},
                    "bgcolor": "#161b22",
                    "borderwidth": 1,
                    "bordercolor": "#30363d",
                    "steps": [
                        {"range": [-8, -4.5], "color": "#3d1010"},
                        {"range": [-4.5, -2.5], "color": "#4a2010"},
                        {"range": [-2.5, -1.5], "color": "#3d2e10"},
                        {"range": [-1.5, 1.5], "color": "#2a2a1a"},  # LATERAL (centro)
                        {"range": [1.5, 2.5], "color": "#1a2e1a"},
                        {"range": [2.5, 4.5], "color": "#0f2a18"},
                        {"range": [4.5, 8], "color": "#0a2414"},
                    ],
                    # Ponteiro principal no valor (parte do conceito 0 = neutro)
                    "threshold": {
                        "line": {"color": cor_ponteiro(valor), "width": 5},
                        "thickness": 0.9,
                        "value": v,
                    },
                },
        ...
        # Arco de preenchimento do 0 até o valor (camada extra)
        # Usa scatter polar-like via shape annotations no domínio do gauge — simplificado:
        # linha de referência no 0 (centro)
        fig.update_layout(
